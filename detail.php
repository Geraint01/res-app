...
$stmt = $db->prepare("SELECT r.*, c.name as category_name, c.color as category_color, c.icon as category_icon FROM recipes r LEFT JOIN categories c ON r.category_id = c.id WHERE r.id = :id");
...
// Related recipes
$related = [];
$relResult = $db->prepare("SELECT r.* FROM recipes r WHERE r.category_id = :cat AND r.id != :id LIMIT 4");
$relResult->bindValue(':cat', $recipe['category_id']);
$relResult->bindValue(':id', $id);
$relRows = $relResult->execute();
while ($row = $relRows->fetchArray(SQLITE3_ASSOC)) $related[] = $row;
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="hero-recipe rounded-3xl p-8 slide-up flex flex-col md:flex-row gap-6 items-center">
                ...
                <div>
                    ...
                    <?php endif; ?>
                    <h1 class="text-3xl md:text-4xl font-bold text-gray-900 font-display mb-2">
            ...
            <div class="grid grid-cols-4 gap-3 slide-up">
                <?php
                $stats = [
                    ['icon'=>'clock','label'=>'Persiapan','value'=>$recipe['prep_time'].' mnt'],
                    ['icon'=>'flame','label'=>'Memasak','value'=>$recipe['cook_time'].' mnt'],
                    ['icon'=>'timer','label'=>'Total','value'=>$totalTime.' mnt'],
                    ['icon'=>'users','label'=>'Porsi','value'=>$recipe['servings'].' porsi'],
                ];
                foreach ($stats as $s):
                ?>
                <div class="bg-white rounded-2xl p-4 text-center border border-orange-100 shadow-sm">
                    <i data-lucide="<?= $s['icon'] ?>" class="w-5 h-5 mx-auto mb-2 text-orange-400"></i>
                    <p class="text-xs text-gray-500 mb-1"><?= $s['label'] ?></p>
                    <p class="text-sm font-bold text-gray-800"><?= $s['value'] ?></p>
                </div>
                <?php endforeach; ?>
            </div>
            ...
            <div class="bg-white rounded-3xl p-6 border border-orange-100 shadow-sm slide-up">
                ...
                <ol class="space-y-4">
                    ...
                    <li class="step-line flex gap-4 pb-4">
                        <div class="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-white text-sm z-10" style="background: linear-gradient(135deg, #FF6B35, #FFD23F);">
                            <?= $i+1 ?>
                        </div>
                        <div class="pt-2">
                            <p class="text-gray-700 leading-relaxed"><?= htmlspecialchars($step) ?></p>
                        </div>
                    </li>
                    <?php endforeach; ?>
                </ol>
            ...
            <div class="bg-white rounded-2xl p-5 border border-orange-100 shadow-sm">
                ...
                <div class="space-y-3">
                    ...
                    <a href="detail.php?id=<?= $rel['id'] ?>" class="flex items-center gap-3 p-2 rounded-xl hover:bg-orange-50 transition">
                        <div class="w-12 h-12 rounded-xl bg-orange-50 flex items-center justify-center text-2xl flex-shrink-0">
                            <?= $rel['image_emoji'] ?>
                        </div>
                        <div>
